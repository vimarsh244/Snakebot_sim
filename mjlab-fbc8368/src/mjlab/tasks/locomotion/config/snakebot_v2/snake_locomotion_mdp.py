...
def _root_quat(asset: Entity) -> torch.Tensor:
	quat = torch.nan_to_num(
		asset.data.root_link_quat_w, nan=0.0, posinf=0.0, neginf=0.0
	)
...
def _goal_delta_xy(env: ManagerBasedRlEnv) -> torch.Tensor:
	return _get_goal_pos(env) - _get_head_pos_xy(env)
...
def _rotate_vectors_world_to_root(
	...
	quat_rep = (
		root_quat_w.unsqueeze(1).expand(-1, num_vectors, -1).reshape(-1, 4)
	)
...
def joint_efforts(
	...
	asset: Entity = env.scene[asset_cfg.name]
	return torch.nan_to_num(asset.data.actuator_force, nan=0.0, posinf=0.0, neginf=0.0).clamp(
		-120.0, 120.0
	)
...
def progress_reward(env: ManagerBasedRlEnv) -> torch.Tensor:
	...
	progress = (prev_dist - curr_dist).clamp(-0.15, 0.15)
...
def control_cost(env: ManagerBasedRlEnv) -> torch.Tensor:
	return torch.sum(torch.square(env.action_manager.action), dim=1).clamp(0.0, 8.0) * _grace_mask(env)
...
def unstable_motion_termination(
	...
	lin_speed = torch.norm(torch.nan_to_num(lin_vel, nan=0.0, posinf=0.0, neginf=0.0), dim=1)
	ang_speed = torch.norm(torch.nan_to_num(ang_vel, nan=0.0, posinf=0.0, neginf=0.0), dim=1)
	non_finite = (~torch.isfinite(root_pos)).any(dim=1) | (~torch.isfinite(lin_vel)).any(dim=1) | (
		~torch.isfinite(ang_vel)
	).any(dim=1)
